...
: 
  BEGIN OBSERVABLES LB+
    (s1=observable_def_line["O" + $observables_block::observableCounter] 
    {
      observables.add(s1.st);
      $observables_block::observableCounter++;
    } 
    LB+)* 
  END OBSERVABLES LB*
;
...
//  ((STRING | INT) COLON)?
  ...
  -> observables_block(id={upperID},
                       type={$observable_type.text},
                       patterns={$observable_def_line::pattern},
                       name={$s2.text})
;
...
: 
  ...
  -> patterns_block(id={upperID + "_P" + $pattern_list::patternCounter},
                    molecules={$pattern_list::patterns},
                    relationship={$pattern_list::relationships},
                    quantity={$pattern_list::quantity})
;

//similar to rules_species_def, but it doesn't accept stoichiometry factors or a zero
 obs_species_def[String upperID, ReactionAction reactionAction] returns [int stoichiometry,
      Map <String,List<ReactionRegister>> map, String relationship = "", String quantity = ""]
scope{
  List reactants;
  BondList bonds;
}
...
: 
  ...
  -> rule_seed_species_block(id={upperID},
                             molecules={$obs_species_def::reactants},
                             firstBonds={$obs_species_def::bonds.getLeft()},
                             secondBonds={$obs_species_def::bonds.getRight()})
;

relational_observer returns [String relationship, String quantity]
:
  s1=(EQUALS|GT|GTE|LT|LTE) {$relationship = $s1.text;}
  s2=(expression2) {$quantity = $expression2.text;}
;
