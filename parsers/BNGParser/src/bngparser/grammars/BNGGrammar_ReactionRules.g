...
: 
  BEGIN REACTION RULES LB+
    (reaction_rule_def[$reaction_rules_block::reactionCounter] 
    {
...
:  
  (l1=reactionLabel
  {
    $reaction_rule_def::text += $l1.label + ":"; $reaction_rule_def::name = $l1.label;
	}
	)?
  reaction_def[$reaction_rule_def::patternsReactants,
               $reaction_rule_def::patternsProducts,
               "RR" + reactionCounter]
  {
  ...
  -> reaction_block(id={"RR" + reactionCounter},
                    reactant={$reaction_rule_def::patternsReactants},
                    product={$reaction_rule_def::patternsProducts},
                    name={$reaction_rule_def::name},
                    rate={$reaction_rule_def::rateList},
                    bidirectional={bi},
                    //lawType={$rate_function.functionName},
                    leftMap={$reaction_rule_def::reactionAction.getLeft()},
                    rightMap={$reaction_rule_def::reactionAction.getRight()},
                    operations={$reaction_rule_def::reactionAction.getOperations()},
...
reaction_def[List patternsReactants, List patternsProducts, String upperID] returns [boolean bidirectional, 
    Map<String,List<ReactionRegister>> reactionStack, Map<String,List<ReactionRegister>> productStack]
scope{
...
:
  s1=rule_species_def[upperID+"_RP" + $reaction_def::reactantPatternCounter,
  ...
  //(more specifically all the RP's and PP's
  {
    ...
    for(int i=0;i<s1.stoichiometry;i++){
      StringTemplate correctedString = GenericMethods.replace(s1.st,"RP" + counter,
                                                             "RP" + $reaction_def::reactantPatternCounter);
  ...
  s2=rule_species_def[upperID+"_RP"+ $reaction_def::reactantPatternCounter,$reaction_rule_def::reactionAction]
  {
    ...
    for(int i=0;i<s2.stoichiometry;i++){ 
      StringTemplate correctedString = GenericMethods.replace(s2.st,"RP" + counter,
                                                              "RP" + $reaction_def::reactantPatternCounter);
      patternsReactants.add(correctedString);
  ...
  s3=rule_species_def[upperID+"_PP"+ 1,$reaction_rule_def::reactionAction] 
  {
    ...
    for(int i=0;i<s3.stoichiometry;i++){ 
      StringTemplate correctedString = GenericMethods.replace(s3.st,"PP" + counter,
                                                              "PP" + $reaction_def::reactantPatternCounter);
  ...
  s4=rule_species_def[upperID+"_PP"+ $reaction_def::reactantPatternCounter,$reaction_rule_def::reactionAction] 
  {
    ...
    for(int i=0;i<s4.stoichiometry;i++){ 
      StringTemplate correctedString = GenericMethods.replace(s4.st,"PP" + counter,
                                                              "PP" + $reaction_def::reactantPatternCounter);
      patternsProducts.add(correctedString);
      $reaction_def::reactantPatternCounter++;
...
rule_species_def[String upperID, ReactionAction reactionAction] returns [int stoichiometry,
    Map <String,List<ReactionRegister>> map, boolean fixed] throws SemanticException
scope{
  List reactants;
  BondList bonds;
}
...
:
  ...
  -> rule_seed_species_block(id={upperID},
                             molecules={$rule_species_def::reactants},
                             firstBonds={$rule_species_def::bonds.getLeft()},
...
:
  ...
  LPAREN expression[memory] {numArguments++;} 
  (COMMA expression[memory] {numArguments++;})* 
  RPAREN
  {
