...
class ModelComparison:
    ...
    def lookupPatterns(self, graph, center, context, molecule, componentNameIntersection):
        counter = 0
        relevantRules = set([])
    ...
    def convertExperimentNames(self, componentNameIntersection, molecule, experiment, firstFileFlag):
    ...
    def obtainContextDifferences(self, fileName1, fileName2, moleculeNameIntersection, componentNameIntersection, context1, context2):
        ...
        differences, similCounter,totCounter = self.discoverDifferences(contextMatrix1, fileName1, fileName2)
        ...
        for molecule in differences:
            for componentPair in differences[molecule]:
                counter = 0
                hypothesis = self.hypothesisGenerator(componentPair, fileName1, fileName2)
                experimentResults = prettyDict(list)
                for experiment in hypothesis:
                    trueExperiment = self.convertExperimentNames(componentNameIntersection, molecule, experiment, experiment[0] == fileName1)
                    rules = self.lookupPatterns(relationshipGraphDict[trueExperiment[0]], trueExperiment[1], trueExperiment[2] ,molecule, componentNameIntersection)
    ...
    def evaluateDifferences(self, differences, fileName1, fileName2, fileName1raw, fileName2raw):
        for molecule in differences:
...
def compareModelsContext(fileName1, fileName2, context1, context2):
    ...
    moleculeNameIntersection, componentNameIntersection = modelComparison.compareModelsStructurally(fileName1, fileName2, moleculemapping, componentmapping)
