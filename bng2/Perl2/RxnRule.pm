...
sub findMap
{
  ...
  if ( $ean == 0 && $edn == 0 && $scn == 0 && $ccn == 0 ){
    printf "Skipping symmetry factor calculation for %s because all operations syndel.\n", $rr->Name;
    $calc_sym=0;
  } elsif (@{$rr->Reactants}>3){
    printf "Skipping symmetry factor calculation for %s because # reactants > 3.\n", $rr->Name;
    $calc_sym= 0;
  }
  ...
  # TODO: We probably should raise an error if mol add/del is also 0 (a warning is given below)
  if ( $calc_sym ) {
