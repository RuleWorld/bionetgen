...
int main(int argc, char* argv[]){
	...
	try{
		...
		cout << "Unable to open specified file - make sure it exists\n";
		return 1;
	}
	if(string(infileName).size() >= 6){
        if(string(infileName).substr(string(infileName).size()-5,5) == ".bngb")
            string(infileName).replace(string(infileName).size()-5,5,".bngl");
        else
            string(infileName) += ".bngl";
	}
    else
        string(infileName) += ".bngl";
	try{
...
void writeOut(){
	...
	for(int i = 0; i < int(strSpecies.size()); i++)
	...
	for(int i = 0; i < int(strRulesFinal.size()); i++)
