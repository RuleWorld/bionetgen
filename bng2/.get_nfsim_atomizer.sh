...
#  that is knownn to work and that has been squirreled away on the ftp server for the particular platform.
  ...
  if [ "${TRAVIS_OS_NAME}" = "linux" ]; then
    curl -v -u $FTP_USER:$FTP_PASSWORD  -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_travis/NFsim-${TRAVIS_OS_NAME}
...
#    chmod +x $1/sbmlTranslator
    ...
    curl -v -u $FTP_USER:$FTP_PASSWORD  -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_bin-2.3.0/Linux/sbmlTranslator
...
#    chmod +x $1/sbmlTranslator
    ...
    curl -v -u $FTP_USER:$FTP_PASSWORD  -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_bin-2.3.0/MacOSX/sbmlTranslator
    mv       sbmlTranslator               $1/sbmlTranslator
    chmod +x $1/sbmlTranslator
  fi
  ...
  if [ "$2" = "x86" ]; then
    curl -v -u roberthclark:$3 -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_appveyor/build/NFsim-Win32.exe
...
#    mv       sbmlTranslator-Win32.exe  $1/sbmlTranslator.exe
    ...
    curl -v -u roberthclark:$3 -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_bin-2.3.0/Win32/sbmlTranslator.exe
    mv ./sbmlTranslator.exe  $1/sbmlTranslator_64.exe
  fi

  if [ "$2" = "x64" ]; then
    curl -v -u roberthclark:$3 -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_appveyor/build/NFsim-Win64.exe
...
#    mv       sbmlTranslator-Win64.exe  $1/sbmlTranslator.exe
    ...
    curl -v -u roberthclark:$3 -O ftp://ftp.midcapsignals.com/BioNetGen_Beta/d_bin-2.3.0/Win64/sbmlTranslator.exe
