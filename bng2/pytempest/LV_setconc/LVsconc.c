...
{  
    ...
    NV_Ith_S(ratelaws,1) = NV_Ith_S(expressions,1)*NV_Ith_S(species,0)*NV_Ith_S(species,1);
    NV_Ith_S(ratelaws,2) = NV_Ith_S(expressions,2)*NV_Ith_S(species,1);
...
calc_species_deriv ( realtype time, N_Vector species, N_Vector Dspecies, void * f_data )
{
    ...
    NV_Ith_S(Dspecies,1) = NV_Ith_S(ratelaws,1) -NV_Ith_S(ratelaws,2);
...
double* bng_protocol(double* params)
		...
		species_init[0] = 100;
		    ...
		    species_out = integrate(timepoints, species_init, params,nspecies,n_timepoints);
		    for(j=nspecies+1;j<(nspecies+1)*(n_timepoints+1);j++)
		    {
		        species_print[gcounter] = species_out[j];
		        gcounter++;
		    }
