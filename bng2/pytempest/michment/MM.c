...
calc_expressions ( N_Vector expressions, double * parameters )
{
    ...
    NV_Ith_S(expressions,5) = pow(10.0,NV_Ith_S(expressions,0));
    NV_Ith_S(expressions,6) = pow(10.0,NV_Ith_S(expressions,1));
...
{
    NV_Ith_S(observables,0) = NV_Ith_S(species,3);
...
{  
    NV_Ith_S(ratelaws,0) = NV_Ith_S(expressions,5)*NV_Ith_S(species,0)*NV_Ith_S(species,1);
...
double* bng_protocol(double* params)
	    ...
	    int gcounter = 0;
		    ...
		    for(j=0;j<(nspecies+1)*(n_timepoints+1);j++)
		    {
...
int main(int argc,char** argv)
{
    ...
    */
    double params[5] = {-2.77, -1, -2, 6.02, 602};
    int j;
    int counter=0;
    double* species_out = bng_protocol(params);
    int nspecies = 4;
    size_t n_timepoints = 10;
