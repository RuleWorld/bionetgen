...
def SpeciesTypes(model):
	...
	counter = 0
	molecules = [x for x in species]# if all(['listOfInSpeciesTypeBonds' not in i.tag for i in x])]d
	sp = {}
	molID ={}
	cmplx = {}
	id2name = {}
	...
	for i in molecules:
		if all(['listOfInSpeciesTypeBonds' not in x.tag for x in i]): # We are collecting molecule species types only. Not complexes
			molecule_id = dict(i.attrib)[prefix+'id'] #get molecule ID
			if dict(i.attrib)[prefix+'name']:
				molecule_name = dict(i.attrib)[prefix+'name'] #get molecule name if available
			else:
				molecule_name = ''
