...
foreach my $model (@models)
{
	...
	# check SBML import
    if ( -e "${datprefix}_SBML_INPUT.bngl"  and  -e "${outprefix}_SBML_INPUT.bngl" )
    {
        multi_print( " -> checking BNGL generated by SBML importer\n", @allFH );
        my $skipline = '^#';
        my $exit_status = diff_files( "${outprefix}_SBML_INPUT.bngl", "${datprefix}_SBML_INPUT.bngl", $skipline );
        if ($exit_status ne "")
        {   
